...
def ngram_vector(text: str, n: int = 3) -> Dict[str, int]:
    vec: Dict[str, int] = {}
    if len(text) < n:
        if text:
            vec[text] = 1
...
def cosine_similarity(vec1: Dict[str, int], vec2: Dict[str, int]) -> float:
    if not vec1 or not vec2:
        return 0.0
    dot_product = 0
    for gram, count in vec1.items():
        if gram in vec2:
            dot_product += count * vec2[gram]
...
class TextDeduplicator:
    VALID_ALGORITHMS = ("levenshtein", "jaccard", "cosine")
    VALID_MODES = ("exact", "fast", "hybrid")
    ...
    def __init__(
        ...
        self._unique_norm_texts: List[str] = []
        self._unique_fingerprints: List[Set[str]] = []
        self._unique_vectors: List[Dict[str, int]] = []
    ...
    def _compute_similarity(
        ...
        existing_norm: str,
        existing_fp: Set[str],
        existing_vec: Dict[str, int],
    ) -> float:
        if self.algorithm == "jaccard":
            return jaccard_similarity(fp, existing_fp)

        if self.algorithm == "cosine":
            return cosine_similarity(vec, existing_vec)

        if self.mode == "fast":
            return jaccard_similarity(fp, existing_fp)
        elif self.mode == "exact":
            return levenshtein_similarity(norm_text, existing_norm)
        else:
    ...
    def _build_features(self, norm_text: str) -> Tuple[Set[str], Dict[str, int], int]:
        fp = ngram_fingerprint(norm_text, self.ngram_size)
        vec = ngram_vector(norm_text, self.ngram_size) if self.algorithm == "cosine" else {}
        text_len = len(norm_text)
        return fp, vec, text_len

    def is_duplicate(self, text: str, existing_texts: Optional[List[str]] = None) -> bool:
        normalized_text = self._normalize(text)
        fp, vec, text_len = self._build_features(normalized_text)

        if existing_texts is not None:
            for existing in existing_texts:
                norm_existing = self._normalize(existing)
                existing_fp, existing_vec, existing_len = self._build_features(norm_existing)

                if not self._length_pass(text_len, existing_len):
                    continue

                sim = self._compute_similarity(
                    normalized_text, fp, vec,
                    norm_existing, existing_fp, existing_vec
                )
            ...
            for i in range(len(self._unique_texts)):
                ...
                existing_vec = self._unique_vectors[i]
                existing_norm = self._unique_norm_texts[i]

                sim = self._compute_similarity(
                    normalized_text, fp, vec,
                    existing_norm, existing_fp, existing_vec
                )
                if sim >= self.threshold:
                    return True
            return False
    ...
    def add_text(self, text: str) -> bool:
        if not self.is_duplicate(text):
            ...
            fp, vec, text_len = self._build_features(normalized)
            self._unique_texts.append(text)
            self._unique_norm_texts.append(normalized)
            self._unique_fingerprints.append(fp)
            self._unique_vectors.append(vec)
    ...
    def deduplicate_with_details(self, texts: List[str]) -> Tuple[List[str], List[Tuple[str, str, float]]]:
        ...
        for text in texts:
            ...
            if not found_duplicate:
                self._unique_texts.append(text)
                self._unique_norm_texts.append(normalized_text)
                self._unique_fingerprints.append(fp)
                self._unique_vectors.append(vec)
                self._unique_lengths.append(text_len)
